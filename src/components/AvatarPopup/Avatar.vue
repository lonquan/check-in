<template>
  <div class="popup-wrapper">
    <div class="animated-avatar-wrapper">
      <div class="circle circle-1"></div>
      <div class="circle circle-2"></div>
      <div class="circle circle-3"></div>
      <img :src="user.avatar" alt="" class="center-avatar">
    </div>
    <div class="user-info">
      <h1 class="user-name text-shadow text-light">{{ user.name }}</h1>
      <h2 class="user-title text-shadow text-light">{{ user.title }}</h2>
    </div>
  </div>
</template>

<script>
import anime from 'animejs'

export default {
  name: 'Avatar',

  data () {
    return {
      user: {
        id: null,
        name: null,
        avatar: null,
        title: null
      }
    }
  },

  methods: {
    show () {
      const elPopupWrapper = this.$el

      this.animation = anime.timeline({
        delay: 0,
        endDelay: 0,
        easing: 'easeInOutSine',
      }).add({
        targets: elPopupWrapper,
        scale: 0,
        duration: 0
      }).add({
        targets: elPopupWrapper,
        scale: 1,
        duration: 800
      })

      this.animation.finished.then(() => {
        console.log('shown')
      })
    },

    hide () {
      const elPopupWrapper = this.$el

      this.animation = anime.timeline({
        delay: 0,
        endDelay: 0,
        easing: 'easeInOutSine'
      }).add({
        targets: elPopupWrapper,
        scale: 0,
        duration: 500
      }, 0)

      this.animation.finished.then(() => {
        console.log('hidden')
        this.$destroy()
      })
    }
  },

  destroyed () {
    this.$el.remove()
  }
}
</script>

<style scoped lang="scss">
  $popup-wrapper-width: 450px;
  $popup-wrapper-height: 500px;
  $avatar-size: 200px;
  $animated-avatar-wrapper-size: 220px;
  $center-avatar-size: 60%;

  @keyframes rotate-circle-1 {
    0% {
      transform: rotate(0);
    }

    100% {
      transform: rotate(-1turn);
    }
  }

  @keyframes rotate-circle-2 {
    0% {
      transform: rotate(0);
    }

    25% {
      transform: rotate(0.35turn);
    }

    50% {
      transform: rotate(0.5turn);
    }

    100% {
      transform: rotate(0.95turn);
    }
  }

  .popup-wrapper {
    display: flex;
    flex-direction: column;
    background-color: rgba(0, 0, 0, 0.8);
    border-radius: 20px;
    overflow: hidden;
    position: fixed;
    width: $popup-wrapper-width;
    height: $popup-wrapper-height;
    top: calc(50% - #{$popup-wrapper-height} / 2);
    left: calc(50% - #{$popup-wrapper-width} / 2);
    z-index: 9999;
    justify-content: center;
    align-items: center;
    transform: scale(0);

    .animated-avatar-wrapper {
      display: flex;
      justify-content: center;
      align-items: center;
      width: $animated-avatar-wrapper-size;
      height: $animated-avatar-wrapper-size;
      z-index: 99999;
      position: relative;
      margin-bottom: 20px;

      .center-avatar {
        z-index: 9999;
        width: $center-avatar-size;
        height: $center-avatar-size;
        border-radius: 50%;
        background: #0466bf;
        padding: 4px;
        box-shadow: 0 3px 5px rgba(6, 101, 191, 0.5);
      }

      .circle {
        display: block;
        position: absolute;
        width: 100%;
        height: 100%;
        background-position: center center;
        background-size: cover;
      }

      .circle-1 {
        background-image: url(../../assets/01.png);
        transform: rotate(75deg);
        animation-name: rotate-circle-1;
        animation-duration: 4s;
        animation-iteration-count: infinite;
      }

      .circle-2 {
        background-image: url(../../assets/02.png);
        animation-name: rotate-circle-2;
        animation-duration: 5s;
        animation-iteration-count: infinite;
      }

      .circle-3 {
        background-image: url(../../assets/03.png);
      }
    }

    .user-info {
      flex-direction: column;

      .user-name {
        font-size: 2rem;
        font-weight: bold;
        text-align: center;
      }

      .user-title {
        font-size: 1.35rem;
        font-weight: 400;
        margin-top: 0.5em;
        text-align: center;
      }
    }
  }
</style>
